import{_ as n,c as e,d as a,o as i}from"./app-CBV1l0BL.js";const l="/assets/image-lYwrIDgL.png",d={};function r(c,s){return i(),e("div",null,[...s[0]||(s[0]=[a(`<hr><p>title: Java实现延迟任务的四种方案：从DelayQueue到时间轮算法 date: 2026-04-03 categories:</p><ul><li>Java</li><li>架构设计</li><li>中间件 tags:</li><li>Java</li><li>Redisson</li><li>RocketMQ</li><li>Netty</li><li>延迟队列</li><li>时间轮 author: 剑桥折刀</li></ul><hr><h1 id="java实现延迟任务的四种方案-从delayqueue到时间轮算法" tabindex="-1"><a class="header-anchor" href="#java实现延迟任务的四种方案-从delayqueue到时间轮算法"><span>Java实现延迟任务的四种方案：从DelayQueue到时间轮算法</span></a></h1><p>在业务开发中，延迟任务（Delayed Task）是处理&quot;<strong>现在不做，等一会儿再做</strong>&quot;这类需求的核心手段。它主要用于解耦业务流程、提升系统响应速度以及保证数据的最终一致性。</p><p>简单来说，就是当业务逻辑不需要立即执行，或者需要等待特定条件满足后才执行时，此时就会用到延迟任务。例如：在开发中我们经常会遇到这样的需求：<strong>&quot;在这个时间点做某事&quot;或者</strong>&quot;等待一段时间后做某事&quot;。比如订单超时取消、退款处理、定时提醒等。</p><p>今天我们就来通过代码实战，介绍一下 Java 中实现延迟任务的四种主流方案。</p><h3 id="_1-jdk-原生方案-delayqueue" tabindex="-1"><a class="header-anchor" href="#_1-jdk-原生方案-delayqueue"><span>1. JDK 原生方案：DelayQueue</span></a></h3><p>这是最基础、门槛最低的方案，完全依赖 JDK，不需要第三方中间件。它适合单机环境下的轻量级任务。</p><h4 id="核心原理" tabindex="-1"><a class="header-anchor" href="#核心原理"><span>核心原理</span></a></h4><p><code>DelayQueue</code> 是一个无界阻塞队列，只有在延迟时间到了之后，才能从队列中取出元素。它内部使用了 <code>ReentrantLock</code> 和 <code>Condition</code> 实现线程安全。</p><h4 id="代码落地" tabindex="-1"><a class="header-anchor" href="#代码落地"><span>代码落地</span></a></h4><p>首先，我们需要定义一个任务类，实现 <code>Delayed</code> 接口，并重写 <code>getDelay</code> 方法。</p><p><strong>Java 中</strong> <strong><code>DelayQueue</code></strong> <strong>的核心运作原理：</strong><code>DelayQueue</code> 元素<strong>必须实现</strong> <strong><code>Delayed</code></strong> <strong>接口</strong>，必须重写 <code>getDelay(TimeUnit unit)</code> 方法，用来返回<strong>剩余延迟时间</strong>。同时 <code>Delayed</code> 接口<strong>继承自</strong> <strong><code>Comparable</code></strong>， 必须重写 <code>compareTo</code> 方法，用来<strong>按剩余延迟时间排序</strong>， 从队列取元素时：<strong>只有到期的任务才能被取出</strong>，没到期的拿不到。</p><p>以例子理解原理：</p><p>我们把 <code>DelayQueue</code> 比作一个&quot;智能快递柜&quot;<strong>，那么这段文字就是在讲:</strong>&quot;什么样的包裹能放进这个柜子&quot;<strong>以及</strong>&quot;柜子怎么决定先给谁开门&quot;<strong>。所以任何想放入这个队列的任务（我们可以比喻成</strong>&quot;包裹&quot;<strong>），都必须实现</strong> <strong><code>Delayed</code></strong> <strong>接口。然后用接口中的</strong><code>getDelay</code>**方法，获取队列中剩余任务的延迟时间，查看是否快到期，是否需要紧急处理。</p><p>同时<code>Delayed</code> 接口其实继承了 <code>Comparable</code> 接口。这意味着，放进来的任务不仅要会&quot;报时&quot;，还得会&quot;攀比&quot;，我们实现compareTo方法，它会根据 <code>compareTo</code> 方法的逻辑，自动把任务排好序。<code>getDelay</code> 返回时间<strong>最短</strong>（最着急）的任务，会被排在队列的最前面（堆顶）。</p><p><strong>代码案例：</strong></p><div class="language-Java line-numbers-mode" data-highlighter="prismjs" data-ext="Java" data-title="Java"><pre><code><span class="line">@Data</span>
...
<span class="line">    }</span>
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>优缺点总结：</strong></p><ul><li><strong>优点</strong>：<strong>性能极高</strong>。增加任务的时间复杂度是 O(1)，非常适合每秒百万级的超时任务（如网络连接空闲检测）。</li><li><strong>缺点</strong>：精度取决于 tickDuration；任务是在内存中，不支持持久化（进程挂了任务就没了）。</li></ul><p><img src="`+l+'" alt="四种延迟任务方案对比图"></p>',54)])])}const p=n(d,[["render",r]]),o=JSON.parse('{"path":"/blogs/Javashixianyanchirenwudesizhongfangan：congDelayQueuedaoshijianlunsuanfa.html","title":"Java实现延迟任务的四种方案：从DelayQueue到时间轮算法","lang":"zh-CN","frontmatter":{},"headers":[{"level":3,"title":"1. JDK 原生方案：DelayQueue","slug":"_1-jdk-原生方案-delayqueue","link":"#_1-jdk-原生方案-delayqueue","children":[]},{"level":3,"title":"2. 分布式方案：Redisson","slug":"_2-分布式方案-redisson","link":"#_2-分布式方案-redisson","children":[]},{"level":3,"title":"3. 消息中间件方案：RocketMQ","slug":"_3-消息中间件方案-rocketmq","link":"#_3-消息中间件方案-rocketmq","children":[]},{"level":3,"title":"4. 高性能算法方案：时间轮 (HashedWheelTimer)","slug":"_4-高性能算法方案-时间轮-hashedwheeltimer","link":"#_4-高性能算法方案-时间轮-hashedwheeltimer","children":[]}],"git":{"createdTime":1775228292000,"updatedTime":1776230744000,"contributors":[{"name":"剑桥折刀","email":"3144253125@qq.com","commits":4}]},"filePathRelative":"blogs/Java实现延迟任务的四种方案：从DelayQueue到时间轮算法.md"}');export{p as comp,o as data};
